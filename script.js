...
class Smooth {
  ...
  run() {
    this.data.last.one = math.lerp(this.data.last.one, this.data.current, 0.065);
    this.data.last.one = Math.floor(this.data.last.one * 100) / 100;

    this.data.last.two = math.lerp(this.data.last.two, this.data.current, 0.06);
...
class Transition {
  ...
  createTimeline() {
    ...
    staggerFromTo(this.dom.slices, 0, {
      xPercent: 100 },
    {
      xPercent: 0,
      ease: Expo.easeInOut },
